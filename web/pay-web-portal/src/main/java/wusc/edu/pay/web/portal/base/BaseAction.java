...
import wusc.edu.pay.facade.user.entity.*;
...
import wusc.edu.pay.facade.user.service.*;
...
public class BaseAction extends Struts2ActionSupport {
    ...
    public String validateTradePwd(String tradePwd) {
        ...
        try {
            userTradePwdFacade.validTradePwd(getCurrentUserInfo().getLoginName(), DigestUtils.sha1Hex(tradePwd),
                    lockMinute, errMaxTimes);
    ...
    public void addUserLog(OpeStatusEnum opeStatusEnum, String content) {
        userOperatorLogFacade.createUserOperatorLog(getCurrentUserOperator().getLoginName(),
                getCurrentUserInfo().getUserNo(), opeStatusEnum, getIpAddr(), content);
    }
