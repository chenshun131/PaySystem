...
import com.chenshun.transformer.util.JdbcManager;
...
public class DimensionConverterImpl implements IDimensionConverter {
    ...
    private String[] buildDateSql() {
        // 升序排列，获取第一个
        String querySql = "SELECT `id` FROM `dimension_date` WHERE `year` = ? AND `season` = ? AND `month` = ? AND `week` = ? AND `day` = ? AND " +
                "`type` = ? AND `calendar` = ? order by `id`";
