...
import wusc.edu.pay.facade.cost.entity.*;
import wusc.edu.pay.facade.cost.enums.*;
...
public class CalCostOrderBiz extends BaseBizImpl<CalCostOrder> {
    ...
    private boolean createInfoValidate(CalCostOrder order) {
        ...
        if (ValidateUtils.isEmpty(order.getCalInterface())) {
            logger.error("银行接口信息不能为空");
            return false;
        }
        if (ValidateUtils.isEmpty(order.getAmount())) {
            logger.error("交易金额不能为空");
            return false;
        }
        if (ValidateUtils.isEmpty(order.getFromSystem())) {
            logger.error("系统来源不能为空");
            return false;
        }
        if (ValidateUtils.isEmpty(order.getTrxNo())) {
        ...
        if (ValidateUtils.isEmpty(order.getCostItem()) || CostItemEnum.getEnum(order.getCostItem()) == null) {
    ...
    private boolean existOriginalInfo(CalCostOrder order) {
        return !ValidateUtils.isEmpty(order.getOrgTrxNo());
    }
    ...
    private boolean isDoubleOrder(CalCostOrder order) {
        if (order == null || ValidateUtils.isEmpty(order.getTrxNo())) {
            return false;
        }
